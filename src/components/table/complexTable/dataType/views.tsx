...
import { IconViewsSelect } from '../SvgIcons';
import { ViewsChoices, sortTags } from '../ViewsChoices';
...
function ViewsCellContent(props) {
  const {
    colUUID,
    rowUUID,
    dataType,
...
const ViewsCell = React.memo((props) => {
  const {
    colUUID,
    rowUUID,
    onPage,
...
function renderOneColumn(props) {
  ...
  for (let i = 0; i < rows.length; i += 1) {
    if(currentPageRowUUIDs.has(rows[i].uuid)){
      ...
      const td = <ViewsCell key={key} {...props1} />;
      tdList.push(td);
    }
  }
