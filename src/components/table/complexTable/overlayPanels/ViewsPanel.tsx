...
import { ViewsChoices, TagValues, sortTags } from '../ViewsChoices';

function ViewsPanel(props, ref) {
  const {
    options,
    columns,
    setColumns,
  ...
  useEffect(() => {
    if (tagFilter === '') {
      setFilteredChoices(ViewsChoices);
      ...
      if (ViewsChoices) {
        for (const choice of ViewsChoices) {
    ...
    setRows((oldData) => {
      ...
      for (const oldRow of oldData) {
        ...
        if (oldRow.uuid === rowUUID) {
          ...
          if (currentColumn.dataType === 'select') { // 单选
            ...
            if (Array.isArray(oldCellValue)) {
              ...
              if (index === -1) {
                newRow.fields[colUUID] = sortTags([...oldCellValue, tag]);
              }
            } else {
              newRow.fields[colUUID] = [tag];
            }
        ...
        if (oldRow.uuid === rowUUID) {
          ...
          if (Array.isArray(oldCellValue)) {
            ...
            if (index !== -1) {
              ...
              newRow.fields[colUUID] = sortTags(front.concat(back));
            }
          }
        }
  ...
  return (
    <OverlayPanelBox state={panelState} setState={setPanelState}>
      <div className="overlay-viewsPanel">
        {(cellValue?.length > 0
          && (
          ...
          || (
            <div className="chosen-box">
              <span style={{ margin: '0 6px', color: '#6e6e6e' }}>
                请选择至少一个视图
              </span>
            </div>
          )}
        ...
        {/* <div className="input-box">
          <HanziInput placeholder="搜索或输入选项"
            value={tagFilter}
...
export default React.forwardRef(ViewsPanel);
